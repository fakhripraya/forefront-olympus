...
function checkNewPasswordRequestEligibility(
  req,
  res,
  next
) {
  if (!req.session.recoveryInfo)
    return res
      .status(500)
      .send(CANT_VALIDATE_RECOVERY_TOKEN);

  const recoveryInfo = req.session.recoveryInfo.filter(
    (userInfo) => userInfo.token === req.body.recoveryToken
  );
  if (
    !recoveryInfo ||
    Object.keys(recoveryInfo).length === 0
  )
